...
The app does not publish, upload, delete, or move video assets. It writes local handoff files and may create/update one Drive status file named `buda-video-status.json` so decisions survive across agent environments. The skill executes approved work after re-reading decisions.
...
The skill needs Drive access because it reads file names, file metadata, markdown/script text, and cover image bytes for OCR. It also creates or updates `buda-video-status.json` in the configured root folder to remember approvals, channel choices, cover copy, and distribution status.
...
You do not edit this file manually. The skill creates it on the first saved decision and merges it back into the next scan. This is what lets "confirmed distributed", notes, selected output channels, and cover copy survive when the skill is reinstalled or used in another agent environment.
