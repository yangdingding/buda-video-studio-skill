...
These checks are shown in the UI as `口播稿`, `封面图`, and `原始视频`. Missing checks are surfaced as human-readable risks, never as internal keys.
...
- Cover assets: `.png`, `.jpg`, or `.jpeg` image files under folders named `Covers` or `封面`.
