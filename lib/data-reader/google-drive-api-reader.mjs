...
const firstParentId = (parents = []) => (Array.isArray(parents) && parents.length > 0 ? parents[0] : "");
...
const listChildren = async ({ accessToken, folderId }) => {
  ...
  const fields =
    "nextPageToken,files(id,name,mimeType,size,createdTime,modifiedTime,webViewLink,thumbnailLink,parents,owners(displayName,emailAddress),lastModifyingUser(displayName,emailAddress))";
  const files = [];
  let pageToken = "";
...
const decodeTextBuffer = (buffer) => {
  ...
  if (oddNulls > sample.length * 0.18 && oddNulls > evenNulls * 2) return new TextDecoder("utf-16le").decode(buffer);
  if (evenNulls > sample.length * 0.18 && evenNulls > oddNulls * 2) return new TextDecoder("utf-16be").decode(buffer);

  return new TextDecoder("utf-8").decode(buffer);
};

const cleanDownloadedText = (text, maxChars) =>
  text
    .replace(/^\uFEFF/, "")
    .replace(/\r\n/g, "\n")
    .replace(/\t/g, " ")
...
const downloadFileText = async ({ accessToken, fileId, maxChars = 8000 }) => {
  ...
  return cleanDownloadedText(decodeTextBuffer(bytes), maxChars);
};
  ...
  for (const child of children) {
    ...
    if (child.mimeType === folderMimeType) {
      files.push({
        ...
        is_folder: true,
        parent_folder_id: firstParentId(child.parents),
        folder_path: folderPath,
        path: [...folderPath, child.name].join("/"),
        web_view_link: child.webViewLink || "",
        thumbnail_link: child.thumbnailLink || "",
...
export const createGoogleDriveApiReader = ({ config, path, isExample }) => {
  ...
  return {
    ...
    async listVideoItems() {
      ...
      const textSnippetFiles = files.filter((file) => !file.is_folder && [".md", ".srt", ".txt", ".vtt", ".json"].includes(file.extension));
      await Promise.all(
        textSnippetFiles.map(async (file) => {
          const snippet = await downloadFileText({
            accessToken: tokenState.accessToken,
            fileId: file.id,
            maxChars: file.extension === ".md" ? 40000 : file.extension === ".json" ? 16000 : 12000,
          });
          if (snippet) {
