...
const buildItem = async ({ sourceKey, file, related, config, index }) => {
  const matchTitle = normalizeTitle(file.name);
  const filename = filenameFromName(file.name);
  const displayId = displayIdFromName(file.name);
  const title = contentTitleFromName(file.name);
  const transcript = related.transcripts.find((candidate) =>
    normalizeTitle(candidate.name).toLowerCase().includes(matchTitle.toLowerCase()) ||
    matchTitle.toLowerCase().includes(normalizeTitle(candidate.name).toLowerCase())
  );
  const cover = related.covers.find((candidate) =>
    normalizeTitle(candidate.name).toLowerCase().includes(matchTitle.toLowerCase()) ||
    matchTitle.toLowerCase().includes(normalizeTitle(candidate.name).toLowerCase())
  );
  const snippet = await readSnippet(file, 40000);
  const transcriptSnippet = transcript ? await readSnippet(transcript) : "";
  const parsedScript = parseScriptDocument(snippet, { maxRawChars: 40000 });
  ...
  return {
    ...
    script_documents: parsedScript.raw_text
      ? [
          {
            name: file.name,
            path: file.relative_path,
            ...
            modified_at: file.modified_at,
            size: file.size,
            raw_text: parsedScript.raw_text,
            tables: parsedScript.tables,
            sections: parsedScript.sections,
          },
        ]
      : [],
