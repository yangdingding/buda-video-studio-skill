...
const readTopicSourceRows = async (config = {}) => {
  ...
  if (fileId) {
    ...
    return {
      rows: mergeTopicRows(parseCsv(text).filter((row) => getField(row, topicTitleKeys)), repositoryRows),
      sourcePath,
      sourceLabel,
      available: true,
    };
  ...
  if (budaFilePath && (await exists(budaFilePath))) {
    return {
      rows: mergeTopicRows(parseCsv(await readFile(budaFilePath, "utf8")).filter((row) => getField(row, topicTitleKeys)), repositoryRows),
  ...
  if (!csvPath || !(await exists(csvPath))) {
    return {
      rows: mergeTopicRows(repositoryRows),
      sourcePath: "",
      sourceLabel: "",
  ...
  return {
    rows: mergeTopicRows(parseCsv(await readFile(csvPath, "utf8")).filter((row) => getField(row, topicTitleKeys)), repositoryRows),
    sourcePath: csvPath,
    sourceLabel: csvPath,
    available: true,
  };
...
export const topicDataSourceAvailable = async (config = {}) => {
  ...
  if ((await Promise.all(repositoryPathsForConfig(config).map(exists))).some(Boolean)) return true;
