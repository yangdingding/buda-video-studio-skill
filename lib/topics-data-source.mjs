...
const repositoryTopicRows = async (config = {}) => {
  ...
  const patterns = asList(process.env.BUDA_VIDEO_TOPIC_MARKDOWN_PATTERNS || topicSources.markdown_patterns || "apps/*/content/video-ideas/*.md");
  const patternRegexps = patterns.map(globToRegExp);
    ...
    for (const ref of refs) {
      ...
      try {
        files = (await git(repoPath, ["ls-tree", "-r", "--name-only", ref]))
          .split(/\r?\n/)
          .filter((path) => path && patternRegexps.some((regexp) => regexp.test(path)));
      } catch {
        continue;
      }
...
const readTopicSourceRows = async (config = {}) => {
  const topicSources = config.topic_sources || {};
  const repositoryRows = await repositoryTopicRows(config);
  const fileId = topicSourceDriveFileId(topicSources);
  if (fileId) {
    const { text, sourceLabel, sourcePath } = await fetchDriveText({ config, fileId });
    return {
      rows: [...parseCsv(text).filter((row) => getField(row, topicTitleKeys)), ...repositoryRows],
      sourcePath,
      sourceLabel,
      available: true,
    };
  ...
  if (budaFilePath && (await exists(budaFilePath))) {
    return {
      rows: [...parseCsv(await readFile(budaFilePath, "utf8")).filter((row) => getField(row, topicTitleKeys)), ...repositoryRows],
  ...
  return {
    rows: [...parseCsv(await readFile(csvPath, "utf8")).filter((row) => getField(row, topicTitleKeys)), ...repositoryRows],
    sourcePath: csvPath,
    sourceLabel: csvPath,
    available: true,
  };
...
const buildTopicItem = ({ row, index, config, csvPath, existingDecision }) => {
  ...
  const hasScript = scriptDocuments.length > 0;
