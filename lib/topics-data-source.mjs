...
const topicTitleEnKeys = ["title_en", "topic_en", "english_title", "英文标题"];
const topicTitleZhKeys = ["title_zh", "topic_zh", "chinese_title", "中文标题"];
const topicSummaryEnKeys = ["summary_en", "description_en", "english_summary", "英文摘要", "英文简介"];
const topicSummaryZhKeys = ["summary_zh", "description_zh", "chinese_summary", "中文摘要", "中文简介"];
const topicScriptEnKeys = ["script_en", "storyboard_en", "english_script", "英文脚本", "英文分镜"];
const topicScriptZhKeys = ["script_zh", "storyboard_zh", "chinese_script", "中文脚本", "中文分镜"];
const topicTranslationStatusKeys = ["translation_status", "翻译状态"];
const topicSourceRefKeys = ["source_ref", "source_commit", "来源版本", "来源提交"];
...
const buildTopicItem = ({ row, index, config, csvPath, existingDecision }) => {
  ...
  const titleEn = contentTitleFromName(getField(row, topicTitleEnKeys) || rawTopic);
  const titleZh = contentTitleFromName(getField(row, topicTitleZhKeys));
  const topic = titleZh || titleEn;
  const source = getField(row, topicSourceKeys) || "选题表";
  const summaryEn = getField(row, topicSummaryEnKeys) || getField(row, topicDescriptionKeys);
  const summaryZh = getField(row, topicSummaryZhKeys);
  const description = summaryZh || summaryEn;
  const scriptEn = getField(row, topicScriptEnKeys) || getField(row, topicScriptKeys);
  const scriptZh = getField(row, topicScriptZhKeys);
  const scriptText = scriptZh || scriptEn;
  const translationStatus = getField(row, topicTranslationStatusKeys) || (titleZh && summaryZh && scriptZh ? "complete" : "source_only");
  const sourceRef = getField(row, topicSourceRefKeys);
  const scriptDocument = scriptText
    ? {
        ...
        locales: {
          en: scriptEn ? { ...parseScriptDocument(scriptEn), raw_text: scriptEn } : null,
          zh: scriptZh ? { ...parseScriptDocument(scriptZh), raw_text: scriptZh } : null,
        },
      }
    : null;
...
export const normalizeTopicRow = (row) => ({
  ...
  title_en: getField(row, topicTitleEnKeys),
  title_zh: getField(row, topicTitleZhKeys),
  ...
  summary_en: getField(row, topicSummaryEnKeys),
  summary_zh: getField(row, topicSummaryZhKeys),
  script_en: getField(row, topicScriptEnKeys),
  script_zh: getField(row, topicScriptZhKeys),
  translation_status: getField(row, topicTranslationStatusKeys),
  source_ref: getField(row, topicSourceRefKeys),
