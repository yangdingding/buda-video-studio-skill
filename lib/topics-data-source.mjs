...
const readTopicSourceRows = async (config = {}) => {
  ...
  if (fileId) {
    ...
    return {
      rows: parseCsv(text).filter((row) => getField(row, ["topic", "title", "标题", "选题", "主题"])),
      sourcePath,
      sourceLabel,
      available: true,
    };
  ...
  if (budaFilePath && (await exists(budaFilePath))) {
    return {
      rows: parseCsv(await readFile(budaFilePath, "utf8")).filter((row) => getField(row, ["topic", "title", "标题", "选题", "主题"])),
  ...
  return {
    rows: parseCsv(await readFile(csvPath, "utf8")).filter((row) => getField(row, ["topic", "title", "标题", "选题", "主题"])),
    sourcePath: csvPath,
    sourceLabel: csvPath,
    available: true,
  };
...
export const readTopicRows = async (csvPath) => {
  ...
  return parseCsv(await readFile(csvPath, "utf8")).filter((row) => getField(row, ["topic", "title", "标题", "选题", "主题"]));
};
...
export const topicRowId = (row) => getField(row, ["id", "ID"]) || getField(row, ["topic", "title", "标题", "选题", "主题"]);

export const normalizeTopicRow = (row) => ({
  id: getField(row, ["id", "ID"]) || stableId(getField(row, ["topic", "title", "标题", "选题", "主题"])),
  topic: getField(row, ["topic", "title", "标题", "选题", "主题"]),
  status: getField(row, ["status", "状态"]) || "待确认",
  owner: getField(row, ["owner", "负责人"]),
  due_date: getField(row, ["due_date", "dueDate", "交付时间", "截止时间"]),
  priority: getField(row, ["priority", "优先级"]) || "P1",
  source: getField(row, ["source", "来源"]) || "选题表",
  note: getField(row, ["note", "notes", "备注", "说明", "description", "描述"]),
});
