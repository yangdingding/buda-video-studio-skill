...
- Remotion/: AI master video, voice audio, and SRT/VTT subtitles
...
Create the AI review package before any human screen recording. The package must contain a rendered horizontal AI video with picture, voice, and subtitles, plus final cover exports.
...
The AI review gate requires Script/, Remotion/, and Covers/. Do not request or assign a human recording until those files are present and a reviewer approves the AI package.
...
3. Invoke $buda-video-delivery in covers mode with brand profile ${brandProfile}. Produce the selected cover languages and a 9:16 variant when Shorts is selected.
4. Render and verify the master, audio, subtitle, and cover files locally. This handoff does not upload assets to Google Drive yet.
...
The AI package was approved and the human screen recording is the final production input. Put the recording over the AI video where required, create the final horizontal master, then package the selected channel exports.
...
1. Finish the horizontal post-production master in Post/.
2. Invoke $buda-video-delivery in publish mode. It owns subtitle verification, channel packaging, and Shorts generation.
3. Use a native 9:16 composition when the project has one. Use letterbox only as the fallback that preserves a horizontal recording unchanged.
4. Keep platform copy editable in Distribution/; record the published platform and public link only after a human publishes.
5. This handoff does not publish to social platforms or upload assets to Google Drive.
