...
export const outputForChannel = (channel) => ({
  ...
  aspect_ratio:
    channel === "YouTube"
      ? "16:9"
      : channel === "YouTube Shorts" || channel === "视频号"
        ? "9:16"
        : channel === "小红书"
          ? "3:4"
          : "1:1",
  caption: true,
  cover_required: ["YouTube", "小红书", "Bilibili", "视频号"].includes(channel),
  copy_required: true,
});
...
export const deriveCoverCopy = ({ title, snippet, coverText = "" }) => {
  ...
  const lines = String(snippet || "")
    .split(/\r?\n|。|！|？|\. |\? |! /)
    .map(cleanMarkdownLine)
    .filter((line) => line.length >= 6 && line.length <= 52);
...
export const inferProjectStage = ({ rawFiles, voiceoverFiles, scriptFiles, transcriptFiles, coverFiles, youtubeFiles, shortsFiles, videoAccountFiles, requiredChecks, rules = {} }) => {
  ...
  const hasPublishedOutput = youtubeFiles.length > 0 || shortsFiles.length > 0 || videoAccountFiles.length > 0;
  ...
  if (hasPublishedOutput && allRequiredReady && rules.distribution_ready_requires_export !== false) {
    return {
      stage: "distribution_ready",
      status: "to_approve",
      proposedAction: "approve",
      reason: "Cloud Drive has exported channel assets; review distribution and status.",
      rule: "channel_export_found",
      evidence,
    };
  }

  if (hasPublishedOutput && rules.distribution_ready_requires_export !== false) {
    return {
      stage: "distribution_ready",
      status: "needs_review",
      proposedAction: "revise",
      reason: "Cloud Drive has exported channel assets, but required production items are missing.",
      rule: "export_found_missing_required_items",
      evidence,
    };
  }
  ...
  if (hasRaw || hasVoiceover || hasScript || hasTranscript || hasCover) {
    return {
...
export const buildProjectItem = ({ project, files, config, index, readSnippet, readCoverText = () => "" }) => {
  ...
  const isChannelExport = (file) => inFolder(file, youtubeNames) || inFolder(file, shortsNames) || inFolder(file, videoAccountNames);
  ...
  const requiredChecks = [
    {
      key: "voiceover",
      label: "口播稿",
      ready: voiceoverFiles.length > 0,
      count: voiceoverFiles.length,
  ...
  const snippetFile = voiceoverFiles[0] || scriptFiles[0] || transcriptFiles[0];
  const snippet = snippetFile ? readSnippet(snippetFile) : "";
  ...
  return {
    ...
    ref: `Video #${index + 1}`,
    title,
    summary: snippet || `${title}: ${allAssets.length} cloud asset(s) found in Google Drive.`,
    body: snippet,
    ...
    edit_brief: {
      ...
      voiceover_ready: voiceoverFiles.length > 0,
      cover_ready: coverFiles.length > 0,
    },
    cover_copy: {
