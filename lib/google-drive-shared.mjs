...
export const inferProjectStage = ({
  ...
  if (allRequiredReady || (hasRaw && hasDraftVideo && hasEditDirection && rules.ready_for_edit_requires_cover === false)) {
    return {
      stage: "ready_for_edit",
      status: "to_approve",
      proposedAction: "approve",
      reason: "Online Google Drive has script, draft video, and human screen recording ready for overlay export.",
      rule: "required_production_items_ready",
      evidence,
    };
  }

  if (hasScriptOrTranscript && hasDraftVideo && !hasRaw) {
    return {
      stage: "script_ready",
      status: "needs_review",
      proposedAction: "revise",
      reason: "Script and draft video are ready; human screen recording is the final recording step.",
      rule: "draft_video_ready_for_recording",
      evidence,
    };
  }
...
export const buildProjectItem = ({ project, files, config, index, readSnippet, readCoverText = () => "" }) => {
  ...
  const draftVideoNames = library.draft_video_folder_names || ["Draft", "Drafts", "草稿视频", "草稿", "HyperFrames", "HyperFrame", "Preview", "预览"];
  const draftVideoKeywords = library.draft_video_file_keywords || ["draft", "草稿", "hyperframe", "hyperframes", "preview", "rough cut", "roughcut"];
  ...
  const requiredChecks = [
    {
      ...
      hint: "需要 .md 脚本，或 .srt/.vtt/.txt 字幕稿",
    },
    {
      key: "draft_video",
      label: "草稿视频",
      ...
      hint: "需要 HyperFrames 草稿视频，含文字和语音，不需要数字人视频",
    },
    {
      key: "raw_video",
      label: "录屏素材",
      ...
      hint: "需要人类跟着草稿视频完成的 MP4/MOV 录屏素材，导出文件夹除外",
    },
  ];
