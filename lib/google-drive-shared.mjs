...
const isStructuredReferenceLabel = (line) =>
  /^(video\s*)?(title|subtitle|description|summary|标题|视频标题|副标题|摘要|简介)$/i.test(String(line || "").trim());

const extractStructuredReferenceTitle = (value) => {
  const lines = String(value || "").split(/\r?\n/);
  for (let index = 0; index < lines.length; index += 1) {
    const line = cleanMarkdownLine(lines[index]);
    const inlineMatch = line.match(/^(?:video\s*)?(?:title|标题|视频标题)\s*[:：]\s*(.+)$/i);
    if (inlineMatch?.[1]) {
    ...
    if (!/^(?:video\s*)?(?:title|标题|视频标题)$/i.test(line)) continue;
