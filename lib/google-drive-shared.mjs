...
export const inferProjectStage = ({
  ...
  const hasAnyChannelOutput = youtubeFiles.length > 0 || shortsFiles.length > 0 || videoAccountFiles.length > 0;
  const hasRequiredChannelOutputs = youtubeFiles.length >= 2 && shortsFiles.length > 0 && videoAccountFiles.length > 0;
  ...
  if (hasRequiredChannelOutputs && hasFinalCover && rules.distribution_ready_requires_export !== false) {
    return {
      stage: "distribution_ready",
      status: "to_approve",
      proposedAction: "approve",
      reason: "Cloud Drive has YouTube CN/EN, Shorts, video account exports, and final cover; review distribution and status.",
      rule: "required_channel_exports_and_cover_found",
      evidence,
    };
  }

  if (hasRequiredChannelOutputs && rules.distribution_ready_requires_export !== false) {
    return {
      stage: "distribution_ready",
      status: "needs_review",
      proposedAction: "revise",
      reason: "Cloud Drive has YouTube CN/EN, Shorts, and video account exports, but final cover is missing.",
  ...
  if (hasAnyChannelOutput && rules.distribution_ready_requires_export !== false) {
    return {
      ...
      status: "needs_review",
      proposedAction: "revise",
      reason: "Cloud Drive has partial channel exports; wait for YouTube CN/EN, Shorts, and video account outputs.",
