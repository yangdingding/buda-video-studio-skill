...
export const productionProjectExtensions = new Set([
  ".html",
  ".htm",
  ".css",
  ".js",
...
const rowLooksLikeLooseTable = (line) => {
  const text = String(line || "").trim();
  if (!text.includes("|") || markdownTableSeparatorPattern.test(text)) return false;
  return splitMarkdownTableRow(text).filter(Boolean).length >= 3;
};

const inferLooseScriptHeaders = (rows = []) => {
  const width = Math.max(...rows.map((row) => row.length), 0);
  const firstRow = rows[0] || [];
  const first = firstRow[0] || "";
  const second = firstRow[1] || "";
  const third = firstRow[2] || "";
  const fourth = firstRow[3] || "";
  if (/^#|分镜|镜头|序号$/i.test(first) || /时长|时间/i.test(second) || /画面|场景/i.test(third) || /台词|口播/i.test(fourth)) {
    return firstRow.map((cell, index) => cell || `列 ${index + 1}`);
  }
  if (/^\d+$/.test(first) && /\d{1,2}:\d{2}/.test(second)) {
    return ["分镜", "时间", "画面", "台词", ...Array.from({ length: Math.max(0, width - 4) }, (_, index) => `补充 ${index + 1}`)];
  }
  if (/^\d+$/.test(first)) {
    return ["分镜", "画面", "台词", ...Array.from({ length: Math.max(0, width - 3) }, (_, index) => `补充 ${index + 1}`)];
  }
  return Array.from({ length: width }, (_, index) => `列 ${index + 1}`);
};

export const parseScriptDocument = (text, { maxRawChars = 40000 } = {}) => {
  const rawText = String(text || "").replace(/^\uFEFF/, "").replace(/\r\n/g, "\n").trim().slice(0, maxRawChars);
  if (!rawText) return { raw_text: "", tables: [], sections: [] };
  ...
  for (let index = 0; index < lines.length; index += 1) {
    if (!String(lines[index] || "").includes("|") || !markdownTableSeparatorPattern.test(lines[index + 1] || "")) continue;
    const headers = splitMarkdownTableRow(lines[index]);
    const rows = [];
    let cursor = index + 2;
    while (cursor < lines.length && String(lines[cursor] || "").includes("|") && String(lines[cursor] || "").trim()) {
      if (!markdownTableSeparatorPattern.test(lines[cursor])) {
    ...
    if (rows.length > 0) {
      tables.push({ title: "分镜脚本", headers, rows, row_count: rows.length, source_line: index + 1 });
      index = cursor - 1;
    }
  }
  ...
  if (tables.length === 0) {
    for (let index = 0; index < lines.length; index += 1) {
      if (!rowLooksLikeLooseTable(lines[index])) continue;
      const rowLines = [];
      let cursor = index;
      while (cursor < lines.length && rowLooksLikeLooseTable(lines[cursor])) {
        rowLines.push(lines[cursor]);
        cursor += 1;
      }
      if (rowLines.length >= 2) {
        let cellsByRow = rowLines.map(splitMarkdownTableRow);
        let headers = inferLooseScriptHeaders(cellsByRow);
        const firstRowLooksHeader =
          /#|分镜|镜头|序号/i.test(cellsByRow[0]?.[0] || "") ||
          /时长|时间/i.test(cellsByRow[0]?.[1] || "") ||
          /画面|场景/i.test(cellsByRow[0]?.[2] || "") ||
          /台词|口播/i.test(cellsByRow[0]?.[3] || "");
        if (firstRowLooksHeader) cellsByRow = cellsByRow.slice(1);
        const rows = cellsByRow.map((cells) => headers.map((header, cellIndex) => ({ header, value: cells[cellIndex] || "" })));
        if (rows.length) tables.push({ title: "分镜脚本", headers, rows, row_count: rows.length, source_line: index + 1, inferred: true });
        index = cursor - 1;
      }
    }
  }
  ...
  for (const line of lines) {
    ...
    if (heading) {
      currentSection = { level: heading[1].length, title: cleanMarkdownLine(heading[2]), body: "" };
      sections.push(currentSection);
      continue;
    }
    if (currentSection && currentSection.body.length < 3000) {
  ...
  return {
    raw_text: rawText,
    tables,
    sections: sections.map((section) => ({ ...section, body: section.body.slice(0, 3000) })).filter((section) => section.title || section.body),
  };
};
...
const isProductionManifestFile = (file) => {
  const normalized = normalizeName(file.name);
  return (
    normalized === "production-manifest.json" ||
    normalized === "buda-video-production.json" ||
    normalized.endsWith("production-manifest.json") ||
    normalized.endsWith("video-manifest.json")
  );
};

const parseProductionManifest = (file) => {
  const text = String(file.snippet || file.text || "").trim();
  if (!text || file.extension !== ".json" || !isProductionManifestFile(file)) return null;
  try {
    const parsed = JSON.parse(text);
    const repo = parsed.repo || parsed.repository || parsed.videoWorkspace || parsed.video_workspace || "";
    const pr = parsed.pr || parsed.pullRequest || parsed.pull_request || parsed.prUrl || parsed.pr_url || "";
    const commit = parsed.commit || parsed.sha || parsed.sourceCommit || parsed.source_commit || "";
    const engine = parsed.engine || parsed.productionEngine || parsed.production_engine || "";
    const projectPath = parsed.projectPath || parsed.project_path || parsed.path || "";
    const previewUrl = parsed.previewUrl || parsed.preview_url || parsed.r2Url || parsed.r2_url || "";
    const exports = compactObject(parsed.exports || parsed.driveExports || parsed.drive_exports);
...
const isStructuredReferenceLabel = (line) =>
  /^(video\s*)?(title|subtitle|description|summary|标题|视频标题|标题保持|最终标题|副标题|摘要|简介)$/i.test(String(line || "").trim());

const extractStructuredReferenceTitle = (value) => {
  const lines = String(value || "").split(/\r?\n/);
  for (let index = 0; index < lines.length; index += 1) {
    const line = cleanMarkdownLine(lines[index]);
    const inlineMatch = line.match(/^(?:video\s*)?(?:title|标题|视频标题|标题保持|最终标题)\s*[:：]\s*(.+)$/i);
    if (inlineMatch?.[1]) {
      const candidate = cleanMarkdownLine(inlineMatch[1]);
      if (candidate && !isStructuredReferenceLabel(candidate) && candidate.length <= 140) return candidate;
    }
    if (!/^(?:video\s*)?(?:title|标题|视频标题|标题保持|最终标题)$/i.test(line)) continue;
    for (let nextIndex = index + 1; nextIndex < lines.length; nextIndex += 1) {
      const candidate = cleanMarkdownLine(lines[nextIndex]);
      if (!candidate) continue;
      if (isStructuredReferenceLabel(candidate)) break;
      if (candidate.length <= 140 && !/^https?:\/\//i.test(candidate) && !looksLikeGarbledText(candidate)) return candidate;
      break;
    }
  }
  return "";
...
const splitVoiceoverTitleLines = (value) =>
  String(value || "")
    .split(/\r?\n|。|！|？|\. |\? |! /)
    ...
    .filter((line) => {
      ...
      if (isStructuredReferenceLabel(line)) return false;
      if (/^(备注|note|script|口播稿)$/i.test(line)) return false;
      if (/^https?:\/\//i.test(line)) return false;
      return true;
    });

const voiceoverTitleRules = [
  {
    test: /合同|审合同|写合同|generate document|document|word/i,
    zh: "用 AI 写合同、审合同",
    en: "Generate and Review Contracts With AI",
  },
  {
    test: /MBTI|性格|问卷|survey|survy|了解你/i,
    zh: "用 AI 测出并记住你的 MBTI",
    en: "Use AI to Understand Your MBTI",
  },
  {
    test: /视频知识点|结构化笔记|structured notes|structured-notes|video to structured|视频.*笔记|整理.*笔记/i,
    zh: "把视频整理成结构化笔记",
    en: "Turn Videos Into Structured Notes",
  },
  {
    test: /HR|简历|个人网站|personal website|作品网站|resume/i,
    zh: "如何用 AI 搭建个人网站",
    en: "How to Build a Personal Website With AI",
  },
  {
    test: /微信|支付宝|WeChat|Alipay|订阅|积分|payment/i,
    zh: "用微信和支付宝购买 Buda 订阅与积分",
    en: "Pay for Buda With WeChat Pay and Alipay",
  },
  {
    test: /github|pull request|repository|repo|integration/i,
    zh: "把 GitHub 工作流接入 Buda",
    en: "Connect GitHub Workflows to Buda",
  },
  {
    test: /app.*skill|skill.*app|feat app|app and skill/i,
    zh: "把 App 和 Skill 变成可交付工作流",
    en: "Turn Apps and Skills Into Deliverable Workflows",
  },
  {
    test: /svg|矢量/i,
    zh: "用 AI 生成可编辑 SVG",
    en: "Generate Editable SVGs With AI",
  },
  {
    test: /生日|birthday|image and video|图像|图片.*视频/i,
    zh: "用 AI 生成生日短视频",
    en: "Create Birthday Videos With AI",
  },
  {
    test: /字幕|caption|transcript|srt|whisper/i,
    zh: "用 AI 自动整理字幕稿",
    en: "Clean Up Captions With AI",
  },
  {
    test: /多人|协作|collaboration/i,
    zh: "让 Agent 支持多人协作",
    en: "Make Agents Collaborative",
  },
];

const scoreVoiceoverTitleLine = (line, index) => {
  let score = 0;
  if (/[\u4e00-\u9fff]/.test(line)) score += 3;
  if (/Buda|AI|Agent|自动|生成|整理|工作流|支付|字幕|视频|项目|文件|合同|简历|问卷|笔记|协作/i.test(line)) score += 4;
  if (/^(如何|怎么|怎样|为什么|还在|你觉得|你有|How to\b|How\b|Why\b|What\b|Still\b|Have you\b)/i.test(line)) score += 5;
  if (/不用|别再|一键|从.+到|把.+变成|turn .+ into|with AI|use .+ to/i.test(line)) score += 3;
  if (line.length >= 10 && line.length <= 46) score += 4;
  if (line.length > 70) score -= 4;
  if (/大家好|今天|我们来|欢迎|关注|订阅|谢谢|下面|首先|然后|我来教你/.test(line)) score -= 4;
  if (/^(and|or|to|with|the|a|an)\b/i.test(line)) score -= 5;
  if (isNoiseCoverLine(line)) score -= 6;
  return score - index * 0.08;
};

const conciseTitleFromLine = (line) => {
  const text = normalizeCoverPhrase(line);
  if (!text) return "";
  const question = text.match(/^(.{8,42}?[吗呢么]|.{8,42}?[?？])/);
  if (question?.[1]) return stripTrailingPunctuation(question[1]);
  const aiAction = text.match(/用\s*AI\s*([^，。！？?；;]{2,22})/i);
  if (aiAction?.[1]) return normalizeCoverPhrase(`用 AI ${aiAction[1]}`);
  const howTo = text.match(/(?:如何|怎么|怎样)([^，。！？?；;]{2,28})/);
  if (howTo?.[1]) return normalizeCoverPhrase(`如何${howTo[1]}`);
  const firstClause = text.split(/[，,。！？?；;：:]/).map((part) => part.trim()).find((part) => part.length >= 8 && part.length <= 42);
  if (firstClause) return normalizeCoverPhrase(firstClause);
  if (text.length <= 54) return text;
  return normalizeCoverPhrase(text.slice(0, 52));
};

const titleLikeVoiceoverLine = (line) => {
  const title = conciseTitleFromLine(line);
  if (title.length < 10 || title.length > 48) return "";
  if (/^(如何|怎么|怎样|为什么)/.test(title)) return title;
  if (/^还在/.test(title)) return title;
  if (/[吗么呢]$/.test(title)) return title;
  if (/^(How to\b|Why\b|What\b|Still\b)/i.test(title)) return title;
  return "";
};

const looksLikeWeakHookTitle = (title) => /遇到过|为了找|进度条|拖半天|翻半天|找半天/i.test(String(title || ""));

const synthesizeVoiceoverTitle = (lines, fallbackTitle = "") => {
  const naturalTitle = [...new Set(lines)].map(titleLikeVoiceoverLine).find(Boolean);
  if (naturalTitle && !looksLikeWeakHookTitle(naturalTitle)) return naturalTitle;

  const context = `${lines.slice(0, 18).join(" ")} ${fallbackTitle}`;
  const rule = voiceoverTitleRules.find((item) => item.test.test(context));
  if (rule) return /[\u4e00-\u9fff]/.test(context) ? rule.zh : rule.en;

  const ranked = [...new Set(lines)]
    .map((line, index) => ({ line, score: scoreVoiceoverTitleLine(line, index) }))
    .filter((item) => item.score > 2)
    .sort((a, b) => b.score - a.score);
  const candidate = conciseTitleFromLine(ranked[0]?.line || "");
  if (candidate) return candidate;

  const topic = humanizeTopic(fallbackTitle);
  if (topic && topic !== fallbackTitle) return topic;
  return "";
};

export const summarizeVoiceover = (value, options = {}) => {
  if (looksLikeGarbledText(value)) return "";
  const structuredTitle = extractStructuredReferenceTitle(value);
  if (structuredTitle) return structuredTitle;
  const lines = splitVoiceoverTitleLines(value);
  return synthesizeVoiceoverTitle(lines, options.fallbackTitle) || [...new Set(lines)][0] || "";
};
...
export const inferProjectStage = ({
  ...
  const hasDraftVideo = draftVideoFiles.length > 0;
  ...
  const hasScriptOrTranscript = hasVoiceover || hasEditDirection;
  const hasAiVideoPackage = hasScriptOrTranscript && hasDraftVideo && hasFinalCover;
  const allRequiredReady = requiredChecks.every((check) => check.ready);
  const evidence = {
    raw: rawFiles.length,
    draft_video: draftVideoFiles.length,
    production_project: productionProjectFiles.length,
  ...
  if (hasRaw && hasAiVideoPackage) {
    return {
      stage: "ready_for_edit",
      status: "to_approve",
      proposedAction: "approve",
      reason: "Online Google Drive has the approved AI video package and human screen recording ready for post-production.",
      rule: "required_production_items_ready",
      evidence,
    };
  }

  if (allRequiredReady || hasAiVideoPackage) {
    return {
      stage: "render_ready",
      status: "to_approve",
      proposedAction: "approve",
      reason: "AI video package is ready: script, rendered preview video, voice/subtitles, and cover are available for review.",
      rule: "ai_video_ready_for_review",
      evidence,
    };
  }

  if (hasRaw || hasDraftVideo || productionProjectFiles.length > 0 || hasVoiceover || hasScript || hasTranscript || hasCoverSource || coverFiles.length > 0) {
    return {
...
export const buildProjectItem = ({ project, files, config, index, readSnippet, readCoverText = () => "" }) => {
  ...
  const rawNames = library.raw_folder_names || ["Raw", "原始视频", "原视频", "录屏", "Screen Recording", "Screen Recordings"];
  const draftVideoNames = library.draft_video_folder_names || [
    "Draft",
    "Drafts",
    "草稿视频",
    "草稿",
  ...
  const draftVideoKeywords = library.draft_video_file_keywords || [
    "draft",
    "草稿",
    "hyperframe",
    "hyperframes",
  ...
  const productionProjectNames = library.production_project_folder_names || [
    "HyperFrames",
    "HyperFrame",
    "Remotion",
    "RemotionStudio",
  ...
  const productionProjectKeywords = library.production_project_file_keywords || [
    "hyperframe",
    "hyperframes",
    "remotion",
    "remotionstudio",
  ...
  const looksLikeDraftVideo = (file) => {
    if (inFolder(file, draftVideoNames)) return true;
    const pathText = normalizeName([file.name, file.path, ...(file.folder_path || [])].filter(Boolean).join(" "));
    return draftVideoKeywords.some((keyword) => pathText.includes(normalizeName(keyword)));
  };
  const looksLikeProductionProjectFile = (file) => {
    if (file.is_folder || !productionProjectExtensions.has(file.extension)) return false;
    if (inFolder(file, productionProjectNames)) return true;
    const normalizedName = normalizeName(file.name);
    const pathText = normalizeName([file.name, file.path, ...(file.folder_path || [])].filter(Boolean).join(" "));
    return (
      ["package.json", "remotion.config.ts", "remotion.config.js"].includes(normalizedName) ||
      productionProjectKeywords.some((keyword) => pathText.includes(normalizeName(keyword)))
    );
  };
  const fileIdentity = (file) => file.id || file.path || file.name;

  const draftVideoFiles = files.filter((file) => hasExt(file, videoExtensions) && !isChannelExport(file) && looksLikeDraftVideo(file));
  const productionManifestFiles = files.filter((file) => isProductionManifestFile(file) && file.extension === ".json");
  const productionManifests = productionManifestFiles.map((file) => parseProductionManifest({ ...file, snippet: readSnippet(file) })).filter(Boolean);
  const productionProjectFiles = files.filter((file) => looksLikeProductionProjectFile(file) && !isProductionManifestFile(file));
  const rawFiles = files.filter(
    (file) =>
      hasExt(file, videoExtensions) &&
      !draftVideoFiles.some((draft) => fileIdentity(draft) === fileIdentity(file)) &&
      (inFolder(file, rawNames) || !isChannelExport(file))
  );
  ...
  const requiredChecks = [
    {
      ...
      hint: "选题里需要可审核的剧本/脚本，最好包含分镜、画面和台词",
    },
    {
      key: "draft_video",
      label: "AI 视频",
      ready: draftVideoFiles.length > 0,
      count: draftVideoFiles.length,
      missing_risk: "missing_draft_video",
      hint: "需要 HyperFrames/Remotion 渲染导出的可预览视频，含画面、语音和字幕",
    },
    {
      key: "cover",
      label: "Cover 文件",
      ready: coverFiles.length > 0,
      count: coverFiles.length,
      missing_risk: "missing_cover",
      hint: "需要 Covers/封面 文件夹里的最终 cover 图片",
    },
  ];
  ...
  const allAssets = [
    ...
    ...productionManifestFiles.map((file) => ({ type: "production_manifest", file })),
    ...productionProjectFiles.map((file) => ({ type: "production_project", file })),
    ...draftVideoFiles.map((file) => ({ type: "draft_video", file })),
  ...
  const scriptDocuments = [...new Map([...voiceoverFiles, ...scriptFiles].map((file) => [fileIdentity(file), file])).values()]
    .map((file) => {
      const parsed = parseScriptDocument(readSnippet(file), { maxRawChars: 40000 });
      return {
        name: file.name,
        path: file.path,
    ...
    .filter((document) => document.raw_text);
  const rawSnippet = snippetFile ? readSnippet(snippetFile) : "";
  const snippet = looksLikeGarbledText(rawSnippet) ? "" : rawSnippet;
  const coverTextEntries = [
    ...coverFiles.map((file) => ({ file, sourcePriority: 0 })),
    ...coverSourceFiles.map((file) => ({ file, sourcePriority: 1 })),
  ]
    .map(({ file, sourcePriority }) => ({
    ...
    .sort((a, b) => a.sourcePriority - b.sourcePriority || Number(a.vertical) - Number(b.vertical));
  ...
  const folderTitle = contentTitleFromName(project.name);
  const explicitScriptTitle = extractStructuredReferenceTitle(snippet);
  const title = explicitScriptTitle || folderTitle;
  const displayId = displayIdFromName(title);
  const contentSummary = summarizeVoiceover(snippet, { fallbackTitle: title });
  ...
  return {
    ...
    source_assets: allAssets.map(({ type, file }) => ({
      ...
      size: file.size || 0,
      drive_file_id: file.id,
      preview_url: file.preview_url || file.r2_url || "",
    ...
    production_manifest: productionManifests[0] || null,
    ...
    edit_brief: {
      format: "AI 视频确认后录屏 + 后期剪辑 + 多渠道导出",
      duration_target: inferred.stage === "distribution_ready" ? "按已导出视频为准" : rules.default_long_form_duration || "3-5 分钟长版；30-60 秒短版",
      key_beats: [
        "先确认剧本里的分镜、画面和台词",
        "再确认 AI 视频的画面、语音、字幕和 cover",
        "AI 视频通过后才分配人类录屏",
        "录屏完成后进入后期剪辑和分发导出",
      ],
      transcript_ready: transcriptFiles.length > 0,
      assets_ready: rawFiles.length > 0,
      draft_video_ready: draftVideoFiles.length > 0,
      production_project_ready: productionProjectFiles.length > 0,
      recording_ready: rawFiles.length > 0,
